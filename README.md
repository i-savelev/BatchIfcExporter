...
Используются два механизма из внешней библиотеки `RevitLogger`:

- `RevitLogger.Logger` пишет подробный лог в UTF-8 файл;
- `RevitLogger.DebugWindow` показывает накопленные сообщения в отдельном окне.
...
- `RevitLogger.dll` — внешняя библиотека логирования и debug window: [i-savelev/RevitLogger](https://github.com/i-savelev/RevitLogger)
...
Дополнительные runtime-зависимости для логгера и связанных библиотек, которые должны лежать рядом со сборкой в `output_dll`:
...
- `System.Text.Encoding.CodePages.dll` приходит транзитивно через `RevitServerBrowser.dll`;
- остальные `System.*.dll` подтягиваются как его зависимости;
- `Newtonsoft.Json.dll` используется самим `BatchIfcExporter`.
...
- логирование и debug window теперь подключаются из внешней библиотеки `RevitLogger`.
